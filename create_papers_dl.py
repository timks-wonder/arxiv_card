...
cls = 'DL'
mcls = 'dl'
papers_path = f'arxiv_data/arxiv_{mcls}_papers.csv'
...
# 更新搜索查询
query = f"cat:cs.{cls} AND submittedDate:[{latest_date_str} TO {today}]"
print(f'爬取范围：{query}')
...
while data:
    ...
    if latest_paper_date < pd.to_datetime(today):
        ...
        # 更新搜索查询
        query = f"cat:cs.{cls} AND submittedDate:[{next_latest_date_str} TO {today}]"
        print(f'\n更新爬取范围：{query}')
...
if data:
    ...
    if os.path.exists(papers_path):
        # 读取现有的CSV文件
        existing_df = pd.read_csv(papers_path)
        # 追加新数据
        combined_df = pd.concat([existing_df, df], ignore_index=True)
        # 保存合并后的数据
        combined_df.to_csv(papers_path, index=False)
    else:
        # 如果文件不存在，直接保存新数据
    ...
    # # 获取并保存最大ID到idx.csv
    # max_id = df['id'].max()
    # pd.DataFrame({'idx': [max_id]}).to_csv(idx_path, index=False)
    # print(f"已更新最大ID {max_id} 到 {idx_path}")
else:
    print("未找到相关论文数据")
